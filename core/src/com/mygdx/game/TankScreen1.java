...
public class TankScreen1 implements Screen {
    ...
    public TankScreen1(MyGdxGame game, WelcomePage welcome){
        this.game = game;
        this.welcome = welcome;
        setTank1(new Texture(Gdx.files.internal("t1.jpeg")));
    ...
    public void render(float delta) {
        ...
        if (Gdx.input.isTouched() && Gdx.input.getX()>Gdx.graphics.getWidth()/2+270 && Gdx.input.getX()<Gdx.graphics.getWidth()/2+270+getRight().getWidth()/8.3 && Gdx.input.getY()<Gdx.graphics.getHeight()/2-20+getRight().getHeight()/8.3f && Gdx.input.getY()>Gdx.graphics.getHeight()/2-20){
            getClick().play();
            TankScreen2 g1 = new TankScreen2(getGame(),this);
            getGame().setScreen(g1);
        }
        if(Gdx.input.isTouched() && Gdx.input.getX()>Gdx.graphics.getWidth()/2+60 && Gdx.input.getX()<Gdx.graphics.getWidth()/2+60+getRight().getWidth()/8.3 && Gdx.input.getY()<Gdx.graphics.getHeight()/2-20+getRight().getHeight()/8.3f && Gdx.input.getY()>Gdx.graphics.getHeight()/2-20){
            getClick().play();
            WelcomePage w1 = new WelcomePage(getGame());
